import {createProvider} from '../did-provider-proto/did-provider-mock/index';
...
describe('factory', () => {
    it('should create ipid with all specification methods', () => {
        const ipid = createProvider(mockIpfs);
    ...
    it('should not create ipid if ipfs is not online', () => {
        expect(() => createProvider({ isOnline: () => false })).toThrow('IPFS node is unavailable');
    });

    it('should account for isOnline not being available (e.g.: using HTTP Client)', () => {
        createProvider({ isOnline: undefined });
    });
});
...
describe('resolve', () => {
    ...
    it('should fail if no ipns record found', async () => {
        ...
        const ipid = createProvider(ipfs);

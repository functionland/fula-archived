 ...
 * @param {string} input
 */
 export const parsePayload = (input: string) => {
 /** @type {UCAN.Payload<C>} */
 const payload: UCAN.Payload = json.decode(base64urlpad.baseDecode(input))
...
const parseProof = (input, context) => {
    ...
    try {
        return /** @type {UCAN.Proof} */ (CID.parse(proof as string))
    } catch (error) {
